...
def _extract_metadata_from_image_file(image_path: str) -> dict:
    ...
    the on_image_logged hook (or for any images regenerated by tools that
    only saved the PNG without going through Fooocus's logger).

    Tries PNG `parameters` text chunk first, then EXIF UserComment for JPEG/WebP.
    Returns a dict suitable for `metadata` field of the manifest entry, or {}.

    - Fooocus JSON scheme → returns the parsed dict directly (already key/value)
    - A1111 string scheme → returns {'parameters': '<the raw string>'} so the
      Copy All Params button still has something to copy and the SPA caption
      can render it as a single 'parameters' row (full A1111 parsing would
      require dragging in modules.meta_parser.A1111MetadataParser — kept
      minimal for now)
    """
    try:
        from PIL import Image
    ...
    if isinstance(params, dict):
        # Coerce nested values to JSON-safe types.
        out = {}
