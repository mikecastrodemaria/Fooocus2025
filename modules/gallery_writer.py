...
_A1111_PARAM_RE = __import__('re').compile(
    r'\s*([A-Za-z][\w \-/]*?):\s*("(?:\\.|[^\\"])*"|\([^)]*\)|<[^>]*>|[^,]*)'
)


def _parse_a1111_string(raw: str) -> dict:
    """Best-effort A1111 metadata parser. Splits the well-known
    `prompt\\nNegative prompt: ...\\nKey: value, Key: value, ...` shape into
    a flat dict (lowercased, snake_case keys). Always preserves the raw
    string under `_raw_a1111` so the Copy button can give it back verbatim.
    """
    out = {'_raw_a1111': raw}
    parts = raw.split('Negative prompt:', 1)
    prompt_part = parts[0].rstrip()
    if prompt_part:
        out['prompt'] = prompt_part
    ...
    # The params line is the LAST line with at least one `Key:` token —
    # negative prompts can themselves span multiple lines.
    params_idx = None
    for i in range(len(lines) - 1, -1, -1):
        if _A1111_PARAM_RE.search(lines[i]):
            params_idx = i
            break
    if params_idx is None:
        if rest:
            out['negative_prompt'] = rest
        return out
    ...
    for m in _A1111_PARAM_RE.finditer(params_line):
        key = m.group(1).strip().lower().replace(' ', '_').replace('-', '_').replace('/', '_')
        val = m.group(2).strip().strip('"').strip()
        if not key or not val:
            continue
        # Normalise common keys to the names the SPA already renders.
        if key == 'cfg_scale':       key = 'cfg_scale'
        elif key == 'sampler':       key = 'sampler'
        elif key == 'schedule_type': key = 'scheduler'
        elif key == 'size':          key = 'resolution'
        elif key == 'model':         key = 'base_model'
        elif key == 'model_hash':    key = 'model_hash'
        elif key == 'clip_skip':     key = 'clip_skip'
        elif key == 'denoising_strength': key = 'denoising_strength'
        elif key == 'version':       key = 'version'
        # Coerce simple types.
        try:
...
def _extract_metadata_from_image_file(image_path: str) -> dict:
    ...
    the on_image_logged hook (or images saved by other tools).

    Two Fooocus metadata schemes supported (per modules/meta_parser.py):
      - **Fooocus JSON** scheme → `parameters` is already a dict.
      - **A1111** scheme → `parameters` is a plain-text string
        `prompt\\nNegative prompt: ...\\nKey: value, Key: value`. We parse it
        via _parse_a1111_string() into the same flat dict shape so the SPA
        renders structured rows (Sampler / CFG / Steps / …) for both
        schemes uniformly. The original raw text is preserved under
        `_raw_a1111` so the Copy button can offer it verbatim.

    Returns a dict suitable for the `metadata` field of the manifest entry,
    or {} on failure (broken file, unknown scheme, etc.).
    """
    try:
        from PIL import Image
        from modules.meta_parser import read_info_from_image
    except Exception:
        return {}
    try:
        with Image.open(image_path) as im:
            params, _scheme = read_info_from_image(im)
    except Exception as e:
        print(f'[asset-browser] could not read metadata from {image_path}: {e}')
        return {}
    ...
    if isinstance(params, str) and params.strip():
        return _parse_a1111_string(params)
    return {}
...
def _build_image_entry(image_path: str, metadata=None, task=None) -> dict:
    ...
    `metadata` may be:
      - A list of (label, key, value) tuples (the format `private_logger.log()`
        passes to the on_image_logged hook). We turn it into a flat dict.
      - A dict (already-flat metadata, e.g. backfilled by reindex via
        _extract_metadata_from_image_file).
      - None — try to extract from the image file on disk so the lightbox
        always has SOMETHING to show, even for old PNGs.
    """
    ...
    if isinstance(metadata, dict):
        # Already-flat dict — accept as-is.
        ...
        # List of (label, key, value) tuples — flatten to {key: value}.
        try:
